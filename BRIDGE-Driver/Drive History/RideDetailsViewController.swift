...
class RideDetailsViewController: UITableViewController, CLLocationManagerDelegate {
    ...
    override func viewDidLoad() {
        ...
        ref?.child("drivers").child(userID).child("history").child(selectedRide).observe(.value, with: { (snapshot) in
            if let rideDetails = snapshot.value as? [String: AnyObject] {
                self.driverName.text = rideDetails["riderName"] as? String
                ...
                let usersProfileRef = self.storeRef?.child("images").child("profiles").child("\(rideDetails["riderID"] as! String).png")
